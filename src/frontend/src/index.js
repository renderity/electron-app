...

		{
			...
			if (renderer.exists && webgpu_adapter)
			{
				...
				useWebgpu = () =>
				{
					...
					// 	UniformBlock.getInstance(wasm_wrapper.Addr(wasm_wrapper.exports.surface_uniform_block_camera.value));
					...
					// const surface_material = Material.getInstance(surface_material_addr, Material.ShaderUsage.GLSL_VULKAN, [ scene.descriptor_set ]);
...
`
	...
	void main (void)
	{
		...
		for (;;)
		{
			...
			(
				getRayBoxIntersection(ray, box_min, box_max, intersection_box) &&
				vdist(ray.origin, intersection_box) < nearest_ray_triangle_intersection
			)
			{
				...
				if (box_count === 0)
				{
					...
					for (uint i = 0; i < triangle_count; ++i)
					{
						...
						float ray_origin_to_intersection_distance = vdist(ray.origin, intersection);
